...
export interface TrophyDetailProps {
  selectedTrophy: Trophy;
  totalPlatinums: number;
}

export function TrophyDetailContent({ selectedTrophy, totalPlatinums }: TrophyDetailProps) {
            ...
            <div className="pt-2.5 border-t border-[#1E2740]">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-[#FFD700]" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                  #{selectedTrophy.order} <span className="text-xs text-[#8A9BB8]">of {totalPlatinums}</span>
                </span>
              </div>
            </div>
          </div>
...
export interface MosaicTileListProps {
...
export function MosaicTileList({ displayTrophies, selectedTile, onSelectTile, onReorder }: MosaicTileListProps) {
  return (
    <div>
